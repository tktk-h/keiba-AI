...
- `build_race_report` で各馬に評価スコアを、乖離馬に `reasons` を付与。
- モデル・Web UI・データ収集・既存の予想/買い目ロジックは変更しない。
...
## 成功基準
- `python main.py <race_id> --enrich` で、全馬一覧(評価±5・予想勝率・市場勝率・
  複勝率・確信度) + 乖離馬(|評価|≥1)の非オッズ根拠 + 参考扱いの買い目、が表示される。
- 評価スコア・予想勝率・根拠がすべて同一モデル由来で矛盾しない。
- 全テスト緑。
